...
class C64Screen
{
    ...
    printVidmem ( color )
    {
        let pixelPos = { "x": this.xToScreen( FIRSTX40-8 ), "y": FIRSTY25-1 },
            currentLine = 0,
            colCount = 0;
        for ( let i = 0; i < 1000; i++ )
        {
            let screenCode = this.vidmem.load( this.vidmem.base + i ),
                charOffset = screenCode * 8;
            colCount++;
            if ( colCount == 40 )
            {
                currentLine += 1;
                pixelPos["x"] = this.xToScreen( FIRSTX40-8 );
                colCount = 0;
            }
            else
            {
