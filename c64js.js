...
function Palette () {
    return [ "#000000", "#ffffff", "#bc5241", "#8feffb", "#b956eb", "#7edb40", "#553fe4", "#ffff77", "#c17b1d", "#826300", "#f49486", "#727272", "#a4a4a4", "#cdff98", "#9e8dff", "#d5d5d5" ];
}
...
Vic.prototype.read = function ( register ) {
    let _ = this;
    if ( register >= _.base )
    {
        register -= _.base;
...
Vic.prototype.write = function ( register, value ) {
    let _ = this;
    if ( register >= _.base )
    {
        register -= _.base;
...
function ScreenCanvas ( id ) {
    let _ = this;
    _.id = id;
    _.elem = document.getElementById( _.id );
    _.context = _.elem.getContext( "2d" );
    _.outputText = new OutputArea( "output" );
    ...
    _.palette = new Palette();
    _.init();
}

ScreenCanvas.prototype.init = function () {
    let _ = this,
        elem = _.elem,
        parent = elem.parentElement,
        parentStyle = getComputedStyle( parent ),
        parentWidth = parseFloat( parentStyle.width ) - parseFloat( parentStyle.paddingLeft ) - parseFloat( parentStyle.paddingRight ),
        scale = parentWidth > 809 ? 2 : 1,
        ctx = _.context;
    elem.width = scale * 405;
    elem.height = scale * 312;
    ctx.scale( scale, scale );
    elem.addEventListener( "click", function () {
        let rect = elem.getBoundingClientRect(),
            x = _.xmap.out[Math.floor( ( event.clientX - rect.left ) / scale ) - 1],
            y = Math.floor( ( event.clientY - rect.top ) / scale ) - 1;
        _.outputText.write ("x: " + x + "\ny: " + y );
    });
}

ScreenCanvas.prototype.plotPixel = function ( x, y, color ) {
    let _ = this,
        xpos = _.xmap.in[x],
        ypos = y,
        fill = _.palette[color],
        ctx = _.context;
    if ( ( ( x < 0 ) || ( x > 404 ) ) || ( ( y < 0 ) || ( y > 312 ) ) )
    {
        return;
    }
    ctx.fillStyle = fill;
...
function C64Screen ( canvas ) {
    let _ = this;
    _.canvas = new ScreenCanvas( canvas );
    _.init( 14, 6, 14 );
}

C64Screen.prototype.init = function ( borderColor, backgroundColor, textColor ) {
    let _ = this,
        c = _.canvas,
        ctx = c.context;
    ctx.fillStyle = c.palette[borderColor];
    ctx.fillRect( 0, 0, 405, 312);
    ctx.fillStyle = c.palette[backgroundColor];
    ctx.fillRect( OFFSETX + 0x18, 0x33, 320, 200);
}
